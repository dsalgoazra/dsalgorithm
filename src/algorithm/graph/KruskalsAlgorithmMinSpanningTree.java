 ...
 * It uses union-find data structures, it tries to find min spanning tree for the number of total vertices -1.
 * To validate MST should contain E-1 adj.
 *
    ...
    public List<Graph.Edge> findMinimumSpanningTree(Graph g){
        ...
        while(mst.size() < g.getVertices().size() && index < edges.size()) { //O(VlogV)
            ...
            if(!fromVertexParent.equals(toVertexParent)) {
                mst.add(e);
                cost += e.getWeight();
                union(fromVertexParent , toVertexParent);
            }
        }
